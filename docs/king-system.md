...
**It does not go through the gateway, and there is no fallback that does.** It
POSTs straight to the Ollama container's own address — the same one `F-6` uses,
which publishes no port and needs no credential. That is not a detail: between
2026-09-06 and 2026-09-12 this script asked the gateway for the local model and
was answered by `antigravity/gemini-pro-agent`, which is the gateway working as
designed and the navigator failing completely. Both premises die when it
happens — the call stops being free, and the task description leaves the host.
Measured the day it was fixed, the same fifteen cases on both paths:

| path | accuracy | latency | served by |
|---|---|---|---|
| gateway | 15/15 = 100% | 6.90 s | `antigravity` — paid |
| **container, direct** | **13/15 = 86%** | **0.94 s** | **`qwen2.5:1.5b-instruct-q4_K_M`** |

The 100% is the honest-looking number and the useless one: it is a measurement
of the model the navigator exists to avoid calling. So `--eval` now settles
**whose** score it is before asking whether the score is good, reading the
`model` field the container returns — Ollama omits that field entirely on a 404,
so it reports what answered rather than echoing what was asked. A foreign
provider or a silent run exits 3 and prints no verdict on the prompt, because in
neither case is the accuracy evidence about this model. `--self-test` pins that
predicate against eight fixtures, two of which are the foreign-provider case.
...
Seven flows, all enabled and published.
...
| `classify_task` | MCP | Which ladder a task deserves — one word: LOCAL, FREE, PAID or WEB | `free-then-local` |

`classify_task` (built 2026-09-12) is `local-router.sh`'s decision layer exposed
as a tool, carrying the same prompt measured at 86% over 15 cases. Four test
runs, one per label, all correct — `PAID` for a Go race condition, `LOCAL` for
extracting emails, `WEB` for a version number, `FREE` for drafting an email —
which is the point: a classifier that always answered `PAID` would have passed
the first test alone. 5.2–13.0 s per call, because `free-then-local` tries
`opencode/big-pickle` before the on-host model; `local-router.sh` answers the
same question in **0.94 s** because it skips the gateway entirely. Use the
script on the host, the flow from anywhere else.
...
| `trace-report.sh` | When you want to know, and it can go red | **Latency by the model that actually served** (p50/p95/max — the call log has no timings), and **who decided the destination**: `gen_ai.system` separates `direct` (served as addressed), `auto` (the gateway's router chose) and `priority` (a combo ladder the caller named). 487 generations in 24 h as of 2026-09-12 — 57.7% direct to the on-host model, 39.4% router-chosen. Ceilings on the warning+error share (10%) and the router-chosen share (90%); exits 1 on a breach, **2 when the backend cannot be read**, never folding "I could not find out" into the reassuring zero. Credential comes from the running collector, not `.env`, because `. ./.env` truncates `Basic <base64>` at the space. **It is not a reroute detector, and its first version wrongly claimed to be** — the span is written after the gateway decides, so no field on it carries what was asked for. `F-4` answers that, from the agent journal, which records both. See `king-mistakes.md` 47 |
| `agent-eval.sh` | Weekly, Sun 04:47, and it can go red | **Whether the agent is RIGHT**, which nothing else here asked. 17 tasks whose answers a string comparison settles — arithmetic, counting, a numeric sort, clock arithmetic, and a prompt-injection task graded like any other wrong answer, since this agent reads web pages. Deterministic grading on purpose: an LLM judge would be one more unverified mechanism standing in for the thing itself. Measured baseline **17/17 twice**, mean 3.45 s, so the floor is **90%** — one wrong of seventeen passes, two do not. Writes `audit/agent-eval.json` (gitignored — a measurement, not a reference point); `F-11` reads it and never runs it, because the audit is run by hand many times a day and must not bill for 17 model calls each time. **It cannot catch a wrong current fact** — see `F-11` and the cross-check rule in `CLAUDE.md` |
...
**Rebuilt 2026-09-12, after the database loss took the table with it.** The
flows came back as rows and the table they write to did not, so `step_2` failed
on every alert — a sink that looks wired and is not, which is the same failure
this section was written to close, arriving by a different road.

Recreated from the five columns documented above (`event`, `provider`,
`received_at` as DATETIME, `detail`, `api_key`). The step's value map is keyed
by FIELD externalId rather than by name, so every key had to be remapped to the
new table's ids — the old ones named fields that no longer exist. Proven on a
payload carrying a failing `antigravity` and a healthy `ollama`: the row named
antigravity alone, so the shaping logic survived the restore intact.

That diagnostic row was then deleted, for the reason below.
...
| **Local-only work can leave the host — now avoidable** | Found 2026-09-05, mitigated 2026-09-06 | A request naming `ollama/...` is served elsewhere when the prompt trips the gateway's content-based reroute — for the sidecar's own prompt that destination is `gemini-3.7-flash-high`, i.e. Google, measured 3 of 3 on 2026-09-05. **A configuration exists that holds the guarantee**: retarget the prompt's `python_interpreter` example (done in `smol_runner.py`) and drop `graph_stats` — narrowed 2026-09-08 from four graph tools to that one, which is now out of the default; the other three cost nothing. Measured: asked `ollama/…`, served `qwen2.5:1.5b-instruct-q4_K_M`, no egress step error. It costs the graph tools and 241 s against ~10 s. Not the default — see §4. Without it the confidentiality use case is conditional, not guaranteed — check `served_by` or `x-omniroute-provider`. Not fixable here: the routing lives in the vendored subtree. `./scripts/check-model-routing.sh` is the detector — it asks for the local model twice, once with an agent-shaped prompt, and prints which provider answered each. Still reproducing as of 2026-09-05. **Caught in the wild by a guard for the first time on 2026-09-12**: `F-4` went red with *1 of 12 recent run(s) did not get the model they asked for*, and the journal row is unambiguous — asked `ollama/qwen2.5:1.5b-instruct-q4_K_M`, `served_by: claude-sonnet-4-6`, `degraded: true`, 3 steps. **A second sighting the same afternoon widened the scope of the fault**: a run that asked for `opencode/big-pickle` — a *paid* model, not the local one — was also served `claude-sonnet-4-6`, `model_overridden: true`. So this is not a local-model problem that happens to cost money. The reroute re-decides any request whose prompt reads as agent or coding work, whatever model it named. Every previous sighting came from someone running a probe on purpose; this one arrived on its own, which is the difference between knowing a fault exists and being told when it happens. It is also what exposed `trace-report.sh` as unable to see it (`king-mistakes.md` 47) — **`F-4` against `/audit/runs.jsonl` is the only instrument here that holds both the asked-for and the served model on one row**, and that is why it is the detector and the traces are not |
...
| **The local router is not local** | Found 2026-09-06, **closed 2026-09-12** | `local-router.sh` asked for `ollama/qwen2.5:1.5b-instruct-q4_K_M` and was served by `antigravity/gemini-pro-agent` — four runs, four matching call-log rows. Both premises of the design were gone: not free at the margin, and the task description left the machine. It still labelled correctly, which is why it went unnoticed for six days. **Closed by taking the gateway off the path entirely**, using the container address `F-6` already reaches Ollama on — no published port, no credential, and no fallback to the gateway, because a fallback is how this became invisible in the first place. Both paths measured side by side on 2026-09-12: <br>gateway — 15/15 = 100%, 6.90 s mean, served by `antigravity`<br>direct — 13/15 = **86%**, **0.94 s** mean, served by `qwen2.5:1.5b-instruct-q4_K_M`<br>The 100% was never the navigator's score. `--eval` now decides **whose** score it is before deciding whether the score is good, and exits 3 rather than print an accuracy that belongs to another model. What remains open is the general lesson, which no code change closes: anything that assumes naming a local model keeps work local is assuming something the gateway does not promise |
...
| **CVE-2025-68121 in the gateway's TLS binary** | Found 2026-09-05, closed 2026-09-11 | **Closed, by a patch layer rather than a rebuild.** `omniroute:base` was built 2026-08-27 carrying `tls-client-linux-ubuntu-amd64-1.15.1.so`. The upstream half of the problem cleared 2026-09-07 when tls-client re-published the legacy asset names, but **eight rebuild attempts on this VPS all failed on memory** — turbopack sizes itself to the cgroup and exceeds it (killed at 87% of the cage at both 3584 MB and 4608 MB), and webpack needs more than a 4096 MB V8 heap ("Ineffective mark-compacts", no kernel OOM). Swap does not help; V8 throws at its own ceiling rather than spilling. So the gateway was cut over to `omniroute:base` + one layer carrying the 1.16.0 binary, verified against the sha256 upstream publishes and loaded through koffi before cutover: container `e482159a1301` → `963966991bbc`, healthy in 6s, 7 API keys intact, `J-4` and `J-5` green. **The cost:** everything else in that image, including its OS packages, is still the 2026-08-27 build, so this is a stopgap the next real rebuild should overwrite. Exposure while it was open was nil — no configured provider loads the library and `/proc/1/maps` never mapped it. `scripts/king-tls-patch.sh` and `scripts/tls-client-pin.txt` hold the detail |
| **§4.2 closed: the agent is callable from all three places** | Proven 2026-09-12 | The Claude Code row had never been demonstrated. It is now, end to end: a tool call from a Claude session reached the Activepieces MCP endpoint, executed `search_web`, and returned the roadmap's own §4.5 benchmark answer — **v2.11.4, June 3 2026** with two sources cited. `ask_free_model` returned the §4.1 baseline answer (391, not prime) in the same pass. Two restore gaps had to close first, both invisible until something called the flows: **no AI provider** was registered in the fresh Neon project (`404 ENTITY_NOT_FOUND — AIProvider custom`), and **the `gateway_alerts` table did not exist**, so `step_2` failed on every alert. The flows were restored as rows; the things they POINT AT were not |
| **The 50% degraded rate is one model** | Measured 2026-09-11 over all 143 journalled runs | Split by model rather than totalled:<br>`ollama/qwen2.5:1.5b` — **100 runs, 66 degraded (66%), 47.6 s/run**<br>`opencode/big-pickle` — 34 runs, 3 degraded (8%), 24.8 s/run<br>`paid-first` — 4 runs, 0 degraded, 14.2 s/run<br>`agy/claude-sonnet-4-6` — 4 runs, 1 degraded, 8.6 s/run<br>The local 1.5B model takes **70% of agent traffic**, degrades **eight times more often** than the alternative, and is **twice as slow** doing it. `king-mistakes.md` 9 already records that it cannot drive an agent loop; this is the same finding with a denominator. **Not changed here**: it is free at the margin and the alternative is not, so which way that trades is a spending decision, not an engineering one. The numbers are what the decision needs |
| **otel-collector is 21 minor versions behind, and that is a decision** | Measured 2026-09-11 | `J-1` (generalised the same day — it had been checking one hardcoded image) reports `0.139.0` against `0.160.0`, and `J-6` reports two advisories with published fixes we are below: `CVE-2026-47256` (path traversal in the **Sentry exporter**, fixed 0.154.0) and `CVE-2026-55701` (**githubreceiver** ignoring `required_headers`, fixed 0.151.0). Neither component appears in `otel-collector/config.yaml`, which is `otlp` → `memory_limiter` → `batch` → `otlphttp`. So the upgrade is hygiene, not a fix, and it is not taken unasked against a working tracing path. **It is proven safe to take**: the config validates clean against 0.160.0 and against the running 0.139.0, with a deliberately-broken config rejected as the control. When wanted, it is one line in `docker-compose.yml` plus `docker compose --profile tracing up -d otel-collector`, and `E-9` says within the hour whether traces still arrive |
| **Six unfixed advisories in `redis:8.6.5`** | Found 2026-09-11 by `J-6` | Three high, and not one has a published fix: `CVE-2026-23479` (use-after-free in the unblock-client flow, *"may lead to remote code execution"*), `CVE-2025-32023` (out-of-bounds write in hyperloglog), `CVE-2025-21605` (unbounded output-buffer growth), plus three medium. All six read `patched_versions: TBD`, so **there is nothing to upgrade to** — moving versions does not close them. Every one is written *"an authenticated user may…"*, and on a Redis with no password any party that can open the socket is that user: `omniroute-redis` has no `--requirepass` and cannot get one from this repo, and it binds `127.0.0.1:6379` on the host, where there are three shell accounts. `ap-redis` does require a password. This does not change the verdict in `scripts/unauthenticated-datastores.txt` — loopback only, `K-3` green, first-party code on that network — it changes what the missing control is holding back, from lateral movement to code execution. The upstream `--requirepass` patch is the fix and is already prepared |
