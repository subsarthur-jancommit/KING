...
| **OmniRoute admin password is lost** | Confirmed 2026-09-04 | Two candidates were tried against `POST /api/auth/login` and both returned `{"error":"Invalid password"}` — the 24-character `INITIAL_PASSWORD` in `omniroute/.env`, and an 11-character value the operator supplied. The live hash is a third value neither of them matches. No OIDC is configured, so password login is the only route in. **While any browser still holds a live dashboard session, use it to set a new password** — the documented recovery otherwise requires deleting the settings and relaunching, which discards the gateway's configuration |
| **Agent tools inactive** | Blocked on the above | The allowlist ships and is deployed, but `agent_tools_active` is `false`. Loading tools needs a `manage`-scoped `OMNIROUTE_MCP_API_KEY`, and minting one needs dashboard auth |
