...
| **The gateway carries a CVE-affected TLS binary that nothing loads** | Found 2026-09-05, scoped 2026-09-10 | `omniroute:base` was built 2026-08-27 and carries `tls-client-linux-ubuntu-amd64-1.15.1.so`, the binary OmniRoute PR #12612 pinned *away from* over CVE-2025-68121. Verified against the advisory rather than that PR: GHSA-h355-32pf-p2xm is **medium, CVSS 4.8**, attack complexity HIGH — a `crypto/tls` session-resumption flaw where a mutated `ClientCAs`/`RootCAs` pool may resume a session it should reject — not the "CVSS 9.8 out-of-bounds read" the PR claims. **The upstream half cleared 2026-09-07** when tls-client re-uploaded the legacy asset names; the build is no longer blocked, and `omniroute-smoke` is green again. Five bounded rebuild attempts on this VPS then failed on memory instead — turbopack needs ~3.11 GB RSS that no flag bounds, webpack ~3.9 GB of V8 heap, against ~4.4 GB free while serving ten containers. **The exposure meanwhile is zero:** the `.so` is loaded lazily by the TLS-impersonation providers `omniroute/Dockerfile:101` names, and none is configured — `provider_connections` holds only `agy`, `ollama-local`, `openrouter`, `tavily-search`, and `/proc/1/maps` showed the library never mapped 3h into the process. So `J-4` stays red because the deployment is not running what `scripts/tls-client-pin.txt` records, and `J-5` asserts the dormancy that makes that red tolerable — going red itself the moment one of those five providers appears |
