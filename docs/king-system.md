...
| `trace-report.sh` | When you want to know, and it can go red | **Local work that left the host.** The traces carry both halves of the question on one row — `name` is what was asked for (`chat ollama/qwen2.5:1.5b-…`) and `model` is what served it — so this is the one instrument that would have caught the 2026-09-06 fault on day one instead of on day six. Also latency p50/p95/max by the model that *actually* served, and the warning+error share against a ceiling (default 10%). Exits 1 on a finding, **2 when the backend cannot be read** — never folding "I could not find out" into the reassuring zero. Credential comes from the running collector, not `.env`, because `. ./.env` truncates `Basic <base64>` at the space. Reads 247 observations in the last 24 h as of 2026-09-12; 0 escapes, and the detector is exercised against the exact shape of the fault so that zero is a measurement |
